...
import { Search, Edit, Trash2, Printer, ArrowUpDown, CheckSquare, Square, ArrowRightLeft } from "lucide-react";
                ...
                {selectedIds.length > 0 && (
                    <div className="flex gap-2 items-center bg-blue-50 px-3 py-1 rounded border border-blue-100">
                        ...
                        <button
                            ...
                            className="flex items-center gap-1 text-sm text-blue-700 hover:text-blue-900 ml-2 font-medium bg-blue-100 px-2 py-1 rounded"
                        ...
                        {products.length === 0 ? (
                            ...
                            products.map((product) => (
                                <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap font-medium">₸ {product.price.toLocaleString("ru-RU")}</td>
                                    <td className="px-6 py-4 text-center">
                                        {product.storefrontProductId ? (
                                            <span className="inline-flex max-w-min items-center justify-center p-1 rounded-full bg-green-100 text-green-600" title={`ID на витрине: ${product.storefrontProductId}`}>
                                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
                                            </span>
                                        ) : (
                                            <span className="inline-flex max-w-min items-center justify-center p-1 font-bold text-gray-300" title="Не в витрине">
                                                —
                                            </span>
                                        )}
                                    </td>
            ...
            {isGroupModalOpen && (
                <GroupStorefrontModal
                    ...
                    onSuccess={() => {
                        ...
                        setSelectedIds([]);
                        alert("Карточка витрины успешно создана. Перейдите во вкладку 'Витрина'.");
                    }}
                />
            )}
            {isTransferModalOpen && (
